...
def prepare_kb_envrioment(raw_kb_path, train_path, dev_path, test_path, test_mode, add_reverse_relations=True):
    ...
    def get_type(e_name):
        if e_name == DUMMY_ENTITY:
            return DUMMY_ENTITY
        if 'nell-995' in data_dir.lower() or 'amazon' in data_dir.lower():
...
def get_seen_queries(data_dir, entity_index_path, relation_index_path,min_seen):
    print("Queries with less than ",min_seen, " frequencies in train data are unseen.")
    ...
    if 'NELL' in data_dir:
        train_path='train.large.triples'
    else:
        train_path='train.triples'
    with open(os.path.join(data_dir, train_path)) as f:
        for line in f:
            e1, e2, r = line.strip().split('\t')
            e1_id = entity2id[e1]
            r_id = relation2id[r]
            seen_queries.add((e1_id, r_id))
            seen_count[(e1_id, r_id)]=+1
    seen_exps = []
    unseen_exps = []
    num_exps = 0
    #with open(os.path.join(data_dir, 'dev.triples')) as f: Used to be dev set, but changed it to test set because for nell.test we don't have a spearate dev set. So I had to train another model on NELL-955 data and didn't have time to do that. so used test set instead.
    ...
    with open(os.path.join(data_dir, 'dev.triples')) as f:
        for line in f:
            ...
            e1_id = entity2id[e1]
            r_id = relation2id[r]
            #if (e1_id, r_id) in seen_count:
            if seen_count.get((e1_id, r_id),0) >= min_seen:
