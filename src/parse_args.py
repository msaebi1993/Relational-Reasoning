...
parser.add_argument('--use_type', type=str, default='notype',
        help='use type embeddings or not- options: type, notype')
parser.add_argument('--agg_method', type=str, default='npmean',
        help='aggrgation method used for creating type embeddings. --options:  npmax, npmean')
parser.add_argument('--prune_method', type=str, default='page_rank',
        help='prunning criteria for pre-computing action space. --options: page_rank , emb')
parser.add_argument('--neigh_encoder', type=str, default=None,
        help='type of neighbor encoder used. --options: nEnc0: no gnn. nEnc1: with gnn')
parser.add_argument('--lin_cat', type=str, default='cat',help='options: lin or cat. Meaning linear layer transofrmation or only concatenations for type embeddings')
parser.add_argument('--e_r_trainable', type=str, default=True,
                 help='set entityt and relation embeddings to trainable')
parser.add_argument('--min_seen', type=int, default=1,
                  help='number of times a quiry needs to be seen in the train set')
parser.add_argument('--parallel', type=int,default=0, help='use parallel processing on multiple GPUS')
# Data
