...
class GraphSearchPolicy(nn.Module):
    def __init__(self, args):
        ...
        self.fn = None
        self.fn_kg = None
        self.neigh_encoder=args.neigh_encoder
    ...
    def transit(self, e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False):
        ...
        if self.relation_only:
            ...
            if self.neigh_encoder in ['nEnc1','nEnc2']:
                E_s = kg.neighbor_encoder(e_s)
                E = kg.neighbor_encoder(e)
            else:
    ...
    def get_action_embedding(self, action, kg):
        ...
        if self.relation_only:
            ...
            ###Chuxu's
            if self.neigh_encoder== 'nEnc2':
                entity_embedding = kg.neighbor_encoder(e)
            else:
    ...
    def define_modules(self):
        if self.relation_only:
            ...
            input_dim = self.history_dim + self.entity_dim + self.relation_dim
        #self.W0 = nn.Linear(self.entity_dim, int(self.entity_dim/2))
        if self.parallel==1:
            self.W1 = nn.DataParallel( nn.Linear(input_dim, self.action_dim))
            self.W2 = nn.DataParallel( nn.Linear(self.action_dim, self.action_dim))
            self.W1Dropout = nn.DataParallel( nn.Dropout(p=self.ff_dropout_rate))
            self.W2Dropout = nn.DataParallel( nn.Dropout(p=self.ff_dropout_rate))
            if self.relation_only_in_path:
                self.path_encoder = nn.DataParallel( nn.LSTM(input_size=self.relation_dim,
                                        hidden_size=self.history_dim,
                                        num_layers=self.history_num_layers,
                                        batch_first=True))
            else:
                 self.path_encoder = nn.DataParallel( nn.LSTM(input_size=self.action_dim,
                                            hidden_size=self.history_dim,
                                            num_layers=self.history_num_layers,
                                            batch_first=True))
        else:
    ...
    def initialize_modules(self):
        if self.parallel==1:
            if self.xavier_initialization:
                nn.init.xavier_uniform_(self.W1.module.weight)
                nn.init.xavier_uniform_(self.W2.module.weight)
                for name, param in self.path_encoder.module.named_parameters():
                    if 'bias' in name:
