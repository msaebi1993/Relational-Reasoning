...
class PolicyGradient(LFramework):
    def __init__(self, args, kg, pn):
        ...
        e_r_trainable=args.e_r_trainable
        #graph neural network
        if 'complex' in fcn_model:
            fn_state_dict = torch.load(args.complex_state_dict_path)
            fn_kg_state_dict = get_complex_kg_state_dict(args,fn_state_dict,data_dir,use_type,agg_method,lin_cat)

        elif 'distmult' in fcn_model:

            fn_state_dict = torch.load(args.distmult_state_dict_path)
            fn_kg_state_dict = get_distmult_kg_state_dict(args,fn_state_dict,data_dir,use_type,agg_method,lin_cat)
        elif 'conve' in fcn_model:
            fn_state_dict = torch.load(args.conve_state_dict_path)
            fn_kg_state_dict = get_conve_kg_state_dict(fn_state_dict,data_dir,use_type,agg_method)
            fn_nn_state_dict = get_conve_nn_state_dict(fn_state_dict)


        if self.parallel==1:
            fn_kg_state_dict['entity_neigh_agg.module.weight'] = kg.entity_neigh_agg.module.weight
            fn_kg_state_dict['entity_neigh_self.module.weight'] = kg.entity_neigh_self.module.weight
            fn_kg_state_dict['entity_neigh_agg.module.bias'] = torch.zeros(args.entity_dim)
            fn_kg_state_dict['entity_neigh_self.module.bias'] = torch.zeros(args.entity_dim)

            kg.load_state_dict(fn_kg_state_dict,strict=False)
            nn.init.zeros_(kg.entity_embeddings.module.weight[0])
            nn.init.zeros_(kg.entity_embeddings.module.weight[1])
            nn.init.zeros_(kg.relation_embeddings.module.weight[0])
            nn.init.zeros_(kg.relation_embeddings.module.weight[1])
            nn.init.zeros_(kg.relation_embeddings.module.weight[2])

            fn_kg_state_dict['entity_neigh_agg.module.bias'].requires_grad=True
            fn_kg_state_dict['entity_neigh_self.module.weight'].requires_grad=True
            fn_kg_state_dict['entity_neigh_self.module.bias'].requires_grad=True

            if e_r_trainable=='True':
                kg.entity_embeddings.module.weight.requires_grad=True
                kg.relation_embeddings.module.weight.requires_grad=True
            else:
                kg.entity_embeddings.module.weight.requires_grad=False
                kg.relation_embeddings.module.weight.requires_grad=False
            ...
            if e_r_trainable=='True':
                kg.entity_embeddings.weight.requires_grad=True
                kg.relation_embeddings.requires_grad=True
            else:
            ...
            fn_kg_state_dict['entity_neigh_agg.bias'] = torch.zeros(args.entity_dim)
            fn_kg_state_dict['entity_neigh_self.bias'] = torch.zeros(args.entity_dim)
            fn_kg_state_dict['entity_neigh_agg.weight'].requires_grad=True

            fn_kg_state_dict['entity_neigh_agg.bias'].requires_grad=True
            fn_kg_state_dict['entity_neigh_self.weight'].requires_grad=True
            fn_kg_state_dict['entity_neigh_self.bias'].requires_grad=True
        ...
        if verbose:
            ...
            for i in range(len(e1)):
                for j in range(output_beam_size):
                    ...
                    pred_e2_reward = self.reward_fun(e1, r, e2, var_cuda(torch.LongTensor([pred_e2s[i][j]]), requires_grad=False))
                    print('beam {}: score = {}, reward = {} \n<PATH> {}'.format(
                         j, float(pred_e2_scores[i][j]), pred_e2_reward, ops.format_path(search_trace, kg)))
                    #print('beam {}: score = {} \n<PATH> {}'.format(
                        #j, float(pred_e2_scores[i][j]), ops.format_path(search_trace, kg)))
                   # path_log.write('beam {}: score = {} \n<PATH> {}'.format(                        j, float(pred_e2_scores[i][j]), ops.format_path(search_trace, kg)))
        with torch.no_grad():
