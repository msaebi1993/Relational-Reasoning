...
class RewardShapingPolicyGradient(PolicyGradient):
    def __init__(self, args, kg, pn, fn_kg, fn, fn_secondary_kg=None):
        ...
        if self.parallel==1:
            fn_kg_state_dict['entity_neigh_agg.module.weight'] = fn_kg.entity_neigh_agg.module.weight
            fn_kg_state_dict['entity_neigh_agg.module.bias'] = torch.zeros(args.entity_dim)
            fn_kg_state_dict['entity_neigh_self.module.weight'] = fn_kg.entity_neigh_self.module.weight
            fn_kg_state_dict['entity_neigh_self.module.bias'] = torch.zeros(args.entity_dim)
        else:
            fn_kg_state_dict['entity_neigh_agg.weight'] = fn_kg.entity_neigh_agg.weight
            fn_kg_state_dict['entity_neigh_agg.bias'] = torch.zeros(args.entity_dim)
            fn_kg_state_dict['entity_neigh_self.weight'] = fn_kg.entity_neigh_self.weight
